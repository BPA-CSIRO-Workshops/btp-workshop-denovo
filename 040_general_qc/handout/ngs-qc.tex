...
\begin{steps}
Here we are demonstrating \texttt{Skewer} to trim a given adapter sequence.
First look at the fastqc matrices for the file \texttt{adaptorQC.fastq.gz}:
\begin{lstlisting}
fastqc -f fastq  adaptorQC.fastq.gz
firefox adaptorQC.fastq_fastqc.html &
\end{lstlisting}
\end{steps}

\begin{note}
Remeber the adaptor clipping should be done before the quality trimming of bases. The order may vary from one tool to another. So make sure if you know how the method you are using works. 
\end{note}

\begin{steps}
We will removed adaptor and do quality based trimming using a single command.
\begin{lstlisting}
